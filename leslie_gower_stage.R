...
#
# Simple stage structure: Just 2 stages for each species
#
###############################################################################
...
mFr=matrix(c(2,2,5,5))   #Mean reproduction rates
sig_Fr= diag(1,4,4) #Covariance matrix
alphas= matrix( 0.5,4,4) #competitive interactions
diag(alphas) = 1
sr=0.9 #Survival
...
Fr=Fr/(matrix(apply(Fr,2,max),ngens,4,byrow=T)) 
colnames(Fr) = c("1A","2A","1B","2B")

#Population matrixes
nrns1A=matrix(0,ngens,1)
nrns1B=matrix(0,ngens,1)

nrns2A=matrix(0,ngens,1)
nrns2B=matrix(0,ngens,1)

nrns2A[1]=0.1; nrns2B[1]=0.1 #Seed species 2 and allow it to establish as resident

for (n in 1:(ngens-1)) {

	#Invasion: Seed or set to near-zero at the appropriate time steps:
	if (n==invasions[2]) { nrns1A[n]=0.0001 }
	#Switch the roles of resident and invader between species 1 and 2
	if (n==invasions[3]) { nrns1A[n]=0.1; nrns1B[n]=0.1;
							nrns2A[n]=0; nrns2B[n]=0   }
	#Second invasion
	if (n==invasions[4]) { nrns2A[n]=0.0001 }


	#Spatially implicit Leslie-Gower model
	nrns1A[n+1] = sr*nrns1A[n]+Fr[n,1]*mFr[1]*nrns1A[n]/(1+alphas[1]*(nrns1[n]*Fr[n,1]+Fr[n,2]*nrns2[n]))
	nrns2A[n+1] = sr*nrns2A[n]+Fr[n,2]*mFr[2]*nrns2A[n]/(1+alphas[2]*(nrns1[n]*Fr[n,1]+Fr[n,2]*nrns2[n]))

	nrns1B[n+1] = (1-sr)+sr*nrns1B[n]+Fr[n,3]*mFr[3]*nrns1B[n]/(1+alphas[1]*(nrns1[n]*Fr[n,1]+Fr[n,2]*nrns2[n]))
	nrns2B[n+1] = sr*nrns2B[n]+Fr[n,4]*mFr[4]*nrns2B[n]/(1+alphas[2]*(nrns1[n]*Fr[n,1]+Fr[n,2]*nrns2[n]))
